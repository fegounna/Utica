...
class TokenGeneratorUnit(nn.Module):
    def __init__(
        ...
        kernel_size = (
            patch_window_size + 1 if patch_window_size % 2 == 0 else patch_window_size
        )
        self.convs = nn.ModuleList(
            [
                Convolution(
                    kernel_size=kernel_size, out_channels=hidden_dim, dilation=1
                )
        ...
        self.linear_encoder = LinearEncoder(
            hidden_dim_scalar_enc * num_scalar_stats + hidden_dim * (num_ts_feats),
            hidden_dim,
        )
    ...
    def forward(self, x):
        ...
        scalar_embeddings = [
            self.scalar_encoders[i](statistics[i]) for i in range(len(statistics))
        ]
...
class ViTUnit(nn.Module):
    def __init__(
        self, hidden_dim, num_patches, depth, heads, mlp_dim, dim_head, dropout, device
    ):
        super().__init__()
    ...
    def forward(self, x, masks=None):
        ...
        if masks is not None:
            if masks.shape != (b, n):
                raise ValueError(
                    f"masks must have shape {(b, n)}, got {tuple(masks.shape)}"
                )
            mask_tok = self.mask_token.view(1, 1, d).to(dtype=x.dtype)
            x = torch.where(masks.unsqueeze(-1), mask_tok.expand(b, n, d), x)
        ...
        x_embeddings = self.norm(x_embeddings)
        cls_token, patch_tokens = unpack(x_embeddings, ps, "b * d")

        return {
            "x_norm_clstoken": cls_token,  # (B, D)
            "x_norm_patchtokens": patch_tokens,  # (B, N, D)
        }
...
class Mantis8M(
    ...
    def forward(self, x, masks=None, is_training=False):
        ...
        if x.dim() == 2:
            ...
            raise ValueError(
                f"Expected x to have 2 or 3 dims, got {x.dim()} with shape {tuple(x.shape)}"
            )
        x_embeddings = self.tokgen_unit(x)
        res = self.vit_unit(x_embeddings, masks=masks)
        if is_training:
            return res
        else:
            return res["x_norm_clstoken"]
