...
def build_kernel_bank(time_length: int) -> List:
    ...
    return [
        ...
        # ConstantKernel(),
        Matern(length_scale=1.0 / time_length, nu=0.5),
        Matern(length_scale=5.0 / time_length, nu=1.5),
    ]
...
def sample_from_gp_prior(
    ...
    cov = np.asarray(kernel(X), dtype=np.float64)
    n = cov.shape[0]

    mean_vec = (
        np.zeros(n, dtype=np.float64)
        if mean_vec is None
        else np.asarray(mean_vec, dtype=np.float64)
    )
    ...
    for _ in range(max_tries):
        try:
            L = np.linalg.cholesky(cov + jitter_i * I)
            z = np.random.randn(n).astype(np.float64)
            return mean_vec + L @ z
        except np.linalg.LinAlgError:
            jitter_i *= 10.0

    # If still failing, last-resort: add a bigger diagonal and retry once more
    L = np.linalg.cholesky(cov + jitter_i * I)
    z = np.random.randn(n).astype(np.float64)
    return mean_vec + L @ z
...
def random_activation(x: np.ndarray, func_type: str = "linear") -> np.ndarray:
    ...
    if (
        func_type == "sigmoid"
    ):  # Avoid overflow issues with large x， if x<0 , return np.exp(x) / (1 + np.exp(x))
        # return np.where(x < 0, np.exp(x) / (1 + np.exp(x)), 1.0 / (1.0 + np.exp(-x)))
        x = np.asarray(x, dtype=np.float64)
        p = 1.0 / (1.0 + np.exp(-np.abs(x)))
    ...
    if func_type == "swish":
        x = np.asarray(x, dtype=np.float64)
        return 0.5 * x * (1.0 + np.tanh(x / 2.0))
    if func_type == "gaussian":
        return np.exp(-(x**2))
    # default: leaky‑ReLU
    alpha = np.random.uniform(0.01, 3)
...
def random_edge_mapping(parents_data: List[np.ndarray]) -> np.ndarray:
    ...
    chosen_func = np.random.choice(
        ["sigmoid", "sin", "mod", "tanh", "softplus", "swish", "gaussian"]
    )
...
def generate_scm_time_series(
    *,
    ...
    for r in root_nodes:
        ...
        node_data[r] = sample_from_gp_prior(kernel=kernel, X=X, mean_vec=mean_vec)
...
def sample_time_series_from_scm(
    *,
    ...
    data = np.zeros((num_features, time_length), dtype=np.float32)

    for i, n in enumerate(chosen_nodes):
        data[i] = node_data[n].astype(np.float32)

    return data


def _single_scm_sample(
    idx: int,
    *,
    time_length: int,
    num_features: int,
    ...
    return sample_time_series_from_scm(
        time_length=time_length,
...
def generate_multiple_scm_time_series(
    *,
    ...
    """
    Returns:
        np.ndarray of shape (num_series, num_features, time_length)
    """

    samples = Parallel(n_jobs=n_jobs, backend="loky")(
        delayed(_single_scm_sample)(
            i,
            time_length=time_length,
            num_features=num_features,
            max_parents=max_parents,
        ...
        for i in tqdm(range(num_series), desc="Generating time series")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help="total number of series to generate",
    )
    parser.add_argument(
        "-L",
        "--time-length",
        type=int,
        default=512,
        help="length of each individual series",
    )
    parser.add_argument(
        "-O", "--output-file", type=str, default=None, help="output file name"
    )
    parser.add_argument(
        "-F",
        "--features",
        type=int,
        default=4,
        help="how many nodes to sample per SCM draw",
    )
    parser.add_argument(
        "-P",
        "--max-parents",
        type=int,
        default=6,
        help="maximum #parents per node in the DAG",
    )
    parser.add_argument(
        "-M", "--num-nodes", type=int, default=18, help="total nodes in the DAG"
    )

    args = parser.parse_args()

    if args.output_file is None:
        args.output_file = f"/Data/yessin.moakher/data_{args.features}.npy"

    out_path = Path(args.output_file)

    # record generate time
    start_time = time()
    ...
    end_time = time()
    print(results.shape)
    print(np.isfinite(results).all())

    np.save(out_path, results)

    print(
        f"Generated {len(results)} time series in {end_time - start_time:.2f} seconds."
    )
